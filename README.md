...
All images are RGB. During training, every image is resized to **256 x 256** and normalized to `[0, 1]`.
...
| **Diffusion (DDPM)** | ~2.7M | Denoising (eps-prediction) | MSE | [README](src/models/diffusion/README.md) |
...
- Local neighborhood aggregation (3x3 avg pooling) -> robust to small spatial shifts
- L2 feature normalization -> balanced distance metrics
- Higher resolution support (512x512 input) -> 64x64 spatial grid (4x more patches)
...
| Category | Baseline AUROC | Enhanced AUROC | Delta | Status |
|---|---|---|---|---|
| **grid** | 0.5437 | 0.7700 | +0.2263 | YES Major improvement |
| **screw** | 0.6390 | 0.6820 | +0.0430 | YES Modest gain |
| **capsules** | 0.7858 | 0.8106 | +0.0248 | YES Modest gain |

Enhanced features especially help grid (4x more patches capture fine textures) and screw (layer 1 captures thread patterns).
...
1. Single global model approx random detection (AUROC 0.5): Training on 27 heterogeneous categories forces generic representations
...
- **Image size**: 256x256 RGB (baseline), 512x512 (enhanced), normalized to [0, 1]
...
- Python >= 3.10
- PyTorch >= 2.0
- CUDA 12.x (recommended - GPU training is 5-10x faster)
